...
battery / display / value), with Groq llama-powered explanations layered on top.

Two deliverables live in this repo:

| Part | Where | What |
|------|-------|------|
| Jupyter notebook app | `notebooks/`, `src/`, `data/` | The graded Anaconda/Jupyter deliverable — data pipeline, recommender, personas, Groq llama explanations, ipywidgets UI |
...
### Deploy the web app with live Groq llama explanations

The static site has Vercel serverless functions at `api/parse.js` and
`api/explain.js`. The free-text description is interpreted by Groq llama, and the
selected catalogue model is explained with grounded specifications. The Groq llama
key stays server-side. In Vercel project settings, add
`Groq llama_API_KEY` as an environment variable for Preview and Production, then
...
Groq llama is temporarily unavailable. Do not put `Groq llama_API_KEY` in `web/` or in
client-side JavaScript.

**Optional second provider — Groq.** Groq llama's free tier is 20 requests/day
...
change needed. To go back to Groq llama, remove `GROQ_API_KEY` and redeploy; env
